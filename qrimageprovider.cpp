...
QImage QRImageProvider::requestImage(const QString &id, QSize *size, const QSize &requestedSize)
{
    ...
    // Проверяем QR-код из текста (m_qrImage)
    if (cleanId == "qr") {
        ...
        if (m_qrImage.isNull()) {
            ...
            QImage defaultImage(350, 350, QImage::Format_RGB32);
    ...
    // Проверяем штрихкод
        ...
        if (m_barcodeImage.isNull()) {
            ...
            QImage defaultImage(350, 120, QImage::Format_RGB32);
            defaultImage.fill(Qt::white);
            return defaultImage;
        }
    ...
    for (const auto &pair : m_batchImages) {
        if (pair.first == cleanId) {
            ...
            if (image.isNull()) {
                ...
                QImage defaultImage(350, pair.first.startsWith("barcode_") ? 120 : 350, QImage::Format_RGB32);
                defaultImage.fill(Qt::white);
                return defaultImage;
            }
            return image;
    ...
    // Проверяем m_historyImages (для истории)
    if (m_historyImages.contains(cleanId)) {
        ...
        if (image.isNull()) {
            ...
            QImage defaultImage(cleanId.startsWith("barcode_") ? 80 : 80, cleanId.startsWith("barcode_") ? 30 : 80, QImage::Format_RGB32);
            defaultImage.fill(Qt::white);
            return defaultImage;
        }
        return image;
    ...
    QImage defaultImage(350, cleanId.startsWith("barcode_") ? 120 : 350, QImage::Format_RGB32);
    defaultImage.fill(Qt::white);
    return defaultImage;
}
...
QImage QRImageProvider::getQrImage() const
{
    return m_qrImage;
}
